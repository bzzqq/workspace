...
p2 = [0.1, 0.2, 0.1, 0.2, 0.1, 0.05]  # 零配件 2 次品率
...
# 计算总成本的函数
def calculate_total_cost(x1, x2, y, z, n):
    # 零配件检测成本
    C_1_detect = x1 * (n * p_f * c_d1)
    C_2_detect = x2 * (n * p_f * c_d2)
    # 成品检测成本
    C_f_detect = y * (n * c_df)
    # 不合格成品的拆解成本
    C_disassemble = z * (n * p_f * c_s)
    # 调换损失
    C_replace = (1 - y) * (n * p_f * c_r)
    # C_replace = (1 - y) * (n * p_f * (c_r + c1 + c2 + c_f))
    # 计算总成本
    total_cost = C_1_detect + C_2_detect + C_f_detect + C_disassemble + C_replace
    return total_cost
...
for i in range(0, 1):
    ...
    for decision in itertools.product([0, 1], repeat=4):
        x1, x2, y, z = decision
        total_cost = calculate_total_cost(x1, x2, y, z, n)
