...
\begin{frame}
    ...
    \begin{minipage}{0.7\linewidth}
        ...
        \[
            \ln C=-kt+\ln C_{0}
        \]
        上述方程表明$\ln C$与$t$呈线性关系，该直线的斜率为$-k$，截距为$\ln C_0$。
        
        当单室模型药物静脉注射给药以后 ， 测得不同时间 $t_i$的血药浓度$C_i(\mathrm{~i=1~,2~,\cdots,n~})$， 以$\ln C$ 对$t$作图，
        \begin{itemize}
          \item 从其斜率求得消除速率常数$k$
          \item 从其截距求得给药后瞬时间的血药浓度$C_0$
        \end{itemize}
    \end{minipage}\hfill
...
\begin{frame}[t]
    ...
    \]
    \begin{example}
        某患者快速静注一单室模型药物 100 mg ， 立即测得血药浓度为 10 mg/ml ， 4 小时后 ，血药浓度为 7.5 mg/ml ，求表观分布容积$V$。
        \tcblower
        \[
            V = \frac{X_0}{C_0} = \frac{100}{10} = 10 \mathrm{mL}
        \]
        \pause
        \red{荒谬的结果！}
    \end{example}
...
\begin{frame}[t]
    \frametitle{}
    \begin{question}
        已知某药的体内变化符合单室模型，其$t_{1/2}$为1.6 h，$V$为\SI{0.28}{L.kg^{-1}}，某一体重为60 kg的患者，期望注射足够的药量，能使血药浓度水平在5 h内保持\SI{10}{mg.ml^{-1}}以上。
    
        问：需要注射多少药量才能使血药浓度水平在5 h内保持\SI{10}{mg.ml^{-1}}以上？
        \tcblower
        \pause
        \[
            \begin{aligned}
...
\begin{frame}
    \frametitle{血药浓度测定存在困难}
    \begin{minipage}{0.68\linewidth}
        一般情况下，血药浓度法是药物动力学研究以及求算药动学参数的首选方法。
    
    但在某些情况下血药浓度测定存在困难， 例如：
    \begin{enumerate}
      \item 血药浓度低，难以准确测定；
      \item 血浆成分对药物测定干扰严重；
    ...
    这时候可以考虑采用尿药排泄数据求算药动学参数 。 

    尿液取样方便 ， 对人体无损伤 ，可以在一定程度上弥补血药浓度法的不足。
    \end{minipage}\hfill
    \begin{minipage}{0.28\linewidth}
        \begin{figure}[htpb]
            \centering
            \includegraphics[width=\linewidth]{害怕}
        \end{figure}
    \end{minipage}
...
\begin{frame}[t]
    \frametitle{}
    \begin{question}
        某单室模型药物给患者静脉注射 200 mg 后 ， 定时收集尿液 ， 测得尿药累积排泄量$X_{\mathrm{u}}$如下表：
        \begin{table}
            \centering
            \begin{tabular}{ccccccccccccccccccc}
                \bottomrule
                $t$(h) & 0 & 1 &  2 & 3 & 6 &  12 \\
                $X_{\mathrm{u}}$(mg) & 0 & 4.02 & 7.77  & 11.26 & 20.41 &  33.88 \\ \hline
                $t$(h) & 24 & 36 & 48  & 60 & 72  \\
                $X_{\mathrm{u}}$(mg)  & 48.63 & 55.05 & 57.84  & 59.06 & 59.58 \\ \toprule
            \end{tabular}
        \end{table}
        如何根据尿药排泄累积量-时间数据计算药动学参数？
    \end{question}
\end{frame}


\begin{frame}
    \frametitle{单室模型药物静脉注射尿药排泄动力学模型}
    \begin{minipage}{0.48\linewidth}
        \begin{center}
            \begin{tikzpicture}
                \draw (0,-0.5) rectangle (1.5,0.5) node[midway] {$X$};
                \draw[->] (1.5,0) -- (3,0) node[above,midway]{$k_\mathrm{e}$} node[right]{$X_\mathrm{u}$};
                \draw[->] (-1.5,0)  node[left]{$X_0$} -- (0,0);
    ...
    \begin{minipage}{0.48\linewidth}
        ...
        总消除速率常数$k= k_\mathrm{e} + k_{\mathrm{nr}}$
    \end{minipage}

    \smallskip
    尿排泄数据求算动力学参数需符合下列条件：
    \begin{enumerate}
      \item 药物在体内主要以原形从尿中排泄；
...
\begin{frame}
    ...
    静脉注射单室模型药物 ， 尿中原形药物产生的速率与体内的药量成正比：
    \[
...
\begin{frame}
    ...
    \end{minipage}\hfill
    \begin{minipage}{0.48\linewidth}
        以$\ln \frac{\mathrm{d}X_{\mathrm{u}}}{\mathrm{d}t}$对$t$作图， 可以得到一条直线。 通过直线的斜率即可求出药物的总消除速率常数$k$。

        直线在纵轴的截距等于$\ln(k_\mathrm{e}X_0)$。 若设该截距为 $\ln{I_0}$ ， 则可求出肾排泄速率常数$k_\mathrm{e}$：
        \[
...
\begin{frame}
    \frametitle{尿药排泄速率与时间的关系 （ 速率法 ）}
    $\frac{\mathrm{d}X_{\mathrm{u}}}{\mathrm{d}t}$理论上应为$t$时刻的瞬时尿药排泄速率 ， 而在实际情况下尿药浓度并不能能反映集尿瞬间的尿药排泄量，而只能反映两次集尿期间的累积排泄药量，因此$\frac{\mathrm{d}X_{\mathrm{u}}}{\mathrm{d}t}$无法准确获得。
    
    实际工作中通常收集在某段时间间隔($\mathrm{~}t_\mathrm{i}- t_{\mathrm{i-1}}$，即$\Delta t$)内的尿液，以该段时间内排泄的原型药量[$(X_\mathrm{u})_\mathrm{i}- (X_\mathrm{u})_{\mathrm{i-1}}$，即$\Delta X_\mathrm{u}$]除以$\Delta t$，得到集尿时间段内的平均尿药排泄速率，$\frac{\Delta X_\mathrm{u}}{\Delta t}$，将其近似地看作该段集尿时间内中点时间$t_\mathrm{c}$的瞬时尿药排泄速率。 于是：
    \[
...
\begin{frame}
    \frametitle{注意事项}
    以$\ln\frac{\Delta X_{\mathrm{u}}}{\Delta t}$对$t_{\mathrm{c}}$作图时 ， 实验数据常因测定误差出现较大的散乱波动 ， 偏离直线较明显 ， 也就是说 ， 速率法对实验测定误差敏感 。 另外 ， 采用尿排泄速率获取药物动力学参数时 ， 收集尿样的时间间隔对药物动力学参数的准确性影响较大。
    \begin{table}
        \centering
        \begin{tabular}{cccc}
            \bottomrule
...
\begin{frame}[t]
    ...
    \begin{question}
        ...
        \[
            \ln \frac{\Delta X_{u}}{\Delta t}=-0.069t_{\mathrm{c}}+0.6212
        \]
         已知该药属单室模型 ， 分布容积为 30L ， 求该药的 $t_{1/2}$，$k_\mathrm{e}$ ，$Cl_{\mathrm{r}}$。
    \end{question}
\end{frame}
...
\begin{frame}[t]
    \frametitle{}
    \begin{question}
        某制药公司开发了一类新型的抗菌药物，现需要进行临床前药动力学研究。研究人员经尾静脉给SD大鼠注射该抗菌药物200 mg，然后采用代谢笼在不同时间收集尿液，并测得尿药排泄累积量$X_\mathrm{u}$。各小组数据见附件。
        \begin{table}
            \centering
            \caption{尿药排泄累积量$-$时间数据}
            \begin{tabular}{ccccccccccccccccccc}
                \bottomrule
                $t$(h) & 0 & 1 &  2 & 3 & 6 &  12 & 24 & 36 & 48  & 60 & 72 \\
                $X_{\mathrm{u}}$(mg) &  &  &   &  &  &  &  &  &  &  &   \\  \toprule
            \end{tabular}
        \end{table}
        问题：试求该药物的$k$，$t_{1/2}$和$k_\mathrm{e}$值 。
    \end{question}
\end{frame}
...
\begin{frame}
    ...
    \]
    式中$(X_{\mathrm{u}}^{\infty}-X_{u})$称为体内经肾待排泄原形药物量 ， 或称亏量 。
\end{frame}
...
\begin{frame}
    \frametitle{尿药排泄量与时间的关系（亏量法）}
    \begin{minipage}{0.48\linewidth}
        单室模型药物静脉注射给药后 ， 以体内经肾待排泄原形药物量 （ 亏量 ） 的常用对数$\ln(X_{\mathrm{u}}^{\infty}-X_{\mathrm{u}})$对时间$t$作图 ， 可得到一条直线 ， 该直线的斜率为$-k$， 截距为$\ln\frac{k_{\mathrm{e}}X_{0}}{k}$。
...
\begin{frame}
    \frametitle{亏量法的特点}
    \begin{enumerate}
      \item 亏量法对药物消除速率的波动和实验测定误差不敏感 ，以$\ln (X_{\mathrm{u}}^{\infty}-X_{\mathrm{u}})$对时间$t$作图时 ， 实验数据点偏离直线不远 ， 比较规则 ， 求得总消除速率常数值较尿药排泄速率法准确 ；
...
\begin{frame}[t]
    ...
    \pause
        \[
            \begin{aligned}
                Cl_{_r}&=\frac{\Delta X_{_u}/\Delta t}C \\
                &=\frac{37500/0.5}{10} \\
                &=7500 \si{mL/h}
                &=7.5 \si{L/h}
            \end{aligned}
...
\begin{frame}[t]
    ...
    \begin{example}
        ...
        \[
            \begin{aligned}
                &k = \ln 2/t_{1/2} = 0.693/16 = 0.043 \\
                &Cl = k\cdot V = 0.043\times 20 = 0.86 \ \mathrm{L/h} \\
                &Cl_{\mathrm{r}} = 60\%Cl = 0.6\times 0.86 = 0.52 \ \mathrm{L/h} \\
                &Cl_{\mathrm{nr}} = Cl - Cl_{\mathrm{r}} = 0.86 - 0.52 = 0.34 \ \mathrm{L/h}
            \end{aligned}
        \]
    \end{example}
\end{frame}
...
\begin{frame}[t]
    ...
    \begin{redblock}{分组任务3}
        ...
        \begin{table}
            ...
            \end{tabular}
        \end{table}
        问题：用亏量法求该药物的动力学参数$k$，$t_{1/2}$，$k_\mathrm{e}$，肾排泄率。
...
\begin{frame}
    \frametitle{血药浓度与时间的关系}
    $\frac{\mathrm{d}X}{\mathrm{d}t}=k_{0}-kX$分离变量，两边积分：
    \[
        \int_0^X \frac { d X } { k _ { 0 } - k X } = \int_0^t d t
    \]
    得 ：
    \[
        \left[-\frac{1}{k}\ln |k_0 - kX|\right]_0^X = t
    \]
    解得 ：
    \[
...
\begin{frame}
    ...
    \begin{minipage}{0.53\linewidth}
        在静脉滴注给药初期 ， 血药浓度迅速上升 ， 随着静脉滴注时间的延长 ， 血药浓度上升速度趋于缓慢 ， 当$t\to\infty$时，$e^{-kt}\to0$，$(1 - e^{-kt})\to1$，血药浓度趋于一个恒定浓度 ， 此时的血药浓度值称为\highlight{稳态血药浓度}或\highlight{坪浓度}， 用$C_\mathrm{ss}$表示 。
        \[
...
\begin{frame}
    \frametitle{达坪时间}
    静脉滴注给药时，达到稳态血药浓度所需要的时间称为达坪时间。
    
    但是达到稳态血药浓度的时间在理论上是无穷大，所以在药学实践中，通常用达到稳态浓度的某一百分比（如 90\%、95\%、99\%） 来定义“达坪时间”。

    \highlight{达坪分数}
    \[
        f_{ss}=\frac{C}{C_{ss}}=\frac{\frac{k_{0}}{kV}(1-e^{-kt})}{\frac{k_{0}}{kV}}=1-\mathrm{e}^{-kt}
    \]
    可见，消除速率常数$k$越大，达坪分数$f_{\mathrm{ss}}$越快趋近于1，达到坪浓度越快。

    \highlight{达坪时间}
    \[
        t=-\frac{1}{k}\ln{(1-f_{_{\mathrm{ss}}})}
    \]
...
\begin{frame}
    \frametitle{达坪分数$f_{\mathrm{ss}}$与半衰期$t_{1/2}$的关系}
    
    若将达到坪浓度某一分数所需的时间$t$以$n$个生物半衰期来表示，即$t=n\cdot t_{1/2}$，又因$k=0.693/t_{1/2}$，则
    \[
        \begin{aligned}
            f_{ss}&=1-\mathrm{e}^{-kt}\\
            &=1-\mathrm{e}^{-0.693/t_{1/2}\cdot n\cdot t_{1/2}}\\
            &= 1- \mathrm{e}^{0.693n}
        \end{aligned}
    \]
    整理，得
    \[
        n=-\frac{\ln(1-f_{\mathrm{ss}})}{0.693}
    \]
    任何药物达到坪浓度某一分数所需要的时间只与生物半衰期的个数有关，与滴注速率$k_0$无关。
\end{frame}
...
\begin{frame}
    ...
    \end{figure}
    \begin{itemize}
      \item 通常认为经过$4\sim5$个半衰期，浓度已达稳态的 $94\% \sim 97\%$，此时可视为达到稳态。
      \item 若需要更精确控制（如毒性范围窄的药物），可能要求达到 99\% 稳态，约需6.65个半衰期​。
    \end{itemize}
\end{frame}
...
\begin{frame}[t]
    ...
    \begin{example}
        ...
        \pause
        \[
            t =-\frac{\ln{(1-f_{_{\mathrm{ss}}})}}k = -\frac{\ln{(1-0.95)}}{\ln(2)/0.5} = 2.16 \ \mathrm{h}
        \]
    \end{example}
...
\begin{frame}[t]
    ...
    \begin{example}
        ...
        \pause
        \[
            C_\mathrm{ss}=\frac{k_{0}}{kV} = \frac{20\times 60}{\frac{\ln(2)}{3.5}\times 50\times 2} = 60.6 \ \mu\mathrm{ g/mL}
        \]
        \[
            C=\frac{k_{0}}{kV}(1-\mathrm{e}^{-kt}) = \frac{20\times 60}{\frac{\ln(2)}{3.5}\times 50\times 2}(1-\exp{-\frac{\ln(2)}{3.5}\times 10}) = 52.23 \ \mu\mathrm{g/mL}
        \]
    \end{example}
\end{frame}
...
\begin{frame}[t]
    ...
    \begin{example}
        ...
        \pause
        \[
            C_\mathrm{ss1}=\frac{k_{0}}{kV} = \frac{150}{1.04\times40} = 3.61 \ \mu\mathrm{ g/mL}
        \]
        \[
            C_\mathrm{ss2}=\frac{k_{0}}{kV} = \frac{300}{1.04\times40} = 7.21 \ \mu\mathrm{ g/mL}
        \]
        所以，以150mg/h的速度进行静脉滴注更合适。
        \[
            k_{0}=C_\mathrm{ss}{kV} = 6\times 1.04\times40 = 249.6 \ \mathrm{ mg/h}
        \]
    \end{example}
\end{frame}
...
\begin{frame}
    ...
    \]
    在停药后不同时间取血，测定血药浓度，以$\ln C$对$t'$作图或进行线性回归，得到一直线或直线回归方程。
    \begin{itemize}
      \item 根据斜率求得总消除速率常数$k$
      \item 根据截距求得表观分布容积$V$
    \end{itemize}
\end{frame}
...
\begin{frame}[t]
    \frametitle{}
    \begin{redblock}{分组任务4：静脉滴注给药动力学}
        给一男性患者以450 mg/h的滴注速度静脉输注某药物，8小时后停止滴注，根据停滴后1，2，3，4，6，8，10h的血药浓度，计算总消除速率常数，稳态血药浓度。(各小组数据见附件)
        \begin{table}
            \centering
            \caption{停滴后血药浓度$-$时间数据}
            \begin{tabular}{ccccccccccccccccccc}
                \bottomrule
                $t$(h) & 1 & 2 &  3 & 4 & 6 &  8 & 10 \\
                $C$(mg/L) &  &  &   &  &  &  &   \\  \toprule
...
\begin{frame}[t]
    ...
    \begin{example}
        ...
        \pause
    \[
        k = \frac{\ln(2)}{t_{1/2}} = \frac{\ln(2)}{3} = 0.231 \ \mathrm{mg/h}
    \]
    \[
        C_0=\frac{k_{0}}{kV}(1-\mathrm{e}^{-kT}) = \frac{30}{0.231\times 10}(1-\mathrm{e}^{-0.231\times 8}) = 10.94 \ \mathrm{mg/L}
    \]
    \[
        C=C_0\mathrm{e}^{-kt'} = 10.94 \mathrm{e}^{-0.231\times 2} = 6.89 \ \mathrm{mg/L}
    \]
    \end{example}
...
\begin{example}
    ...
    \pause
    \[
        X_0^* = VC_\mathrm{ss} = 60\times 2.5 = 150 \ \mathrm{mg}
    \]
    \[
        k_0 = X_{0}^{*}k = X_{0}^{*}\frac{\ln 2}{t_{1/2}} = 150\times \frac{0.693}{50} = 2.08 \ \mathrm{mg/h}
    \]
    \end{example}
\end{frame}
...
\begin{frame}[t]
    ...
    \begin{example}
        ...
        \pause
    \[
        k = \frac{\ln(2)}{t_{1/2}} = \frac{\ln(2)}{6} = 0.1 \ \mathrm{mg/h}
    \]
    (1) 经过4 h
    \[
        C = \frac{X_{0}^{*}}{V}e^{-kt}+\frac{k_{0}}{kV}(1-e^{-kt}) = \frac{200}{50}e^{-0.1\times 4}+\frac{20}{0.1\times 50}(1-e^{-0.1\times 4}) = 4 \ \mathrm{mg/L}
    \]
    (2) 经过8 h
    \[
        C = \frac{X_{0}^{*}}{V}e^{-kt}+\frac{k_{0}}{kV}(1-e^{-kt}) = \frac{200}{50}e^{-0.1\times 8}+\frac{20}{0.1\times 50}(1-e^{-0.1\times 8}) = 4 \ \mathrm{mg/L}
    \]
    \end{example}
\end{frame}
...
\begin{frame}
    ...
    解得
    \[
        t_{\mathrm{max}}=\frac{1}{k_{\mathrm{a}}-k}\ln \frac{k_{\mathrm{a}}}{k}
    \]
...
\begin{frame}
    ...
    两边取对数，得：
    \[
        \ln C_{r}=-k_{\mathrm{a}}t+\ln\frac{k_\mathrm{a}FX_{0}}{V(k_\mathrm{a}-k)}
    \]
    ...
    \begin{example}
        ...
        \begin{table}
            ...
            \begin{tabular}{cccccccccccc}
                \bottomrule
                $t$(h) & 0.5 & 1.0 & 2.0 & 4.0 & 8.0 & 12.0 \\ \hline
                $C$($\mu$g/mL) & 5.36 & 9.95 & 17.18 & 25.78 & 29.78 & 26.63 \\ \bottomrule \bottomrule
                $t$(h) & 18.0 & 24.0 & 36.0 & 48.0 & 72.0 \\ \hline
                $C$($\mu$g/mL) & 19.40 & 13.26 & 5.88 & 2.56 & 0.49 \\ \toprule 
            \end{tabular}
        \end{table}
    \end{example}
\end{frame}
...
\begin{frame}[t]
    \frametitle{}
    \begin{redblock}{分组任务5：血管外给药，残数法}
        某制药公司欲开发一种口服抗肿瘤药物，现通过临床前药动学研究考察该化合物在体吸收行为。研究人员通过灌胃给于beagle犬$X_0$ mg该化合物，并测得不同时间的血药浓度数据（各小组数据见附件）。通过前期研究已知$V=20$ L，试求该化合物的$k$、$k_\mathrm{a}$、$F$、$AUC$、$t_\mathrm{max}$及$C_\mathrm{max}$。
    \end{redblock}
\end{frame}
...
\begin{frame}
    ...
    因$X=VC$：
    \[
        \frac{\mathrm{d}X_{\mathrm{A}}}{\mathrm{d}t}=V\frac{\mathrm{d}C}{\mathrm{d}t}+kVC
    \]
    \[
        \mathrm{d}X_{\mathrm{A}} = V\mathrm{d}C + kVC\mathrm{d}t
    \]
    在时间$0-t$内积分，得：
    \[
...
\begin{frame}
    ...
    \begin{minipage}{0.88\linewidth}
        \begin{itemize}
            ...
            \item[$C_{t}$：] $t$时刻的血药浓度
            \item[$\int_{0}^{t}C\mathrm{d}t$：] $0-t$血药浓度-时间曲线下面积，即$AUC_{0\to t}$
            \item[$\int_{0}^{\infty}C\mathrm{d}t$：] 血药浓度-时间曲线下总面积，即$AUC_{0\to \infty}$
          \end{itemize}
    \end{minipage}
\end{frame}
...
\begin{frame}
    ...
    两边取对数
    \[
        \ln\left[1-\frac{\left(X_{\mathrm{A}}\right)_t}{\left(X_{\mathrm{A}}\right)_{\mathrm{\infty}}}\right]=-k_\mathrm{a}t
    \]
...
\begin{frame}
    \frametitle{梯形法求$AUC_{0\to t}$}
    \begin{figure}[htpb]
        \centering
        \includegraphics[width=.84\linewidth]{梯形法}
    \end{figure}
\end{frame}


\begin{frame}
    \frametitle{求$k\int_{0}^{\infty}C\mathrm{d}t$}
    \[
        \begin{aligned}
            k\int_{0}^{\infty}C\mathrm{d}t&=k(AUC_{0\to \infty}) \\
            &=k(AUC_{0\to t_n}+AUC_{t_n\to\infty}) \\
            &=k\cdot AUC_{0\to t_n} + k\cdot AUC_{t_n\to\infty}\\
            &=k\cdot AUC_{0\to t_n}+C_n
        \end{aligned}
    \]
    根据最后一点实测血药浓度$C_\mathrm{n}$与$k$值，以及梯形法求得最后一点时的$AUC_{0\to t_n}$，可得$k\int_{0}^{\infty}C\mathrm{d}t$。
\end{frame}
...
\begin{frame}
    ...
    \begin{enumerate}
      ...
      \item 计算$t{_i}$时间点的药物吸收分数$\frac{(X_{\mathrm{A}})_{t}}{(X_{\mathrm{A}})_{\infty}}=\frac{C_{t}+k\int_{0}^{t}C\mathrm{d}t}{k\int_{0}^{\infty}C\mathrm{d}t}$
      \item 计算$t_i$时间点的待吸收分数$1-\frac{(X_{\mathrm{A}})_{t}}{(X_{\mathrm{A}})_{\infty}}$，以其对数对$t$作图，求得直线回归方程，根据直线的斜率求得吸收速率常数$k_\mathrm{a}$
    \end{enumerate}
\end{frame}
    ...
    \begin{question}
        ...
        \begin{table}
            ...
            \begin{tabular}{cccccccccccc}
                \bottomrule
                $t$(h) & 0.5 & 1.0 & 2.0 & 4.0 & 8.0 & 12.0 \\ \hline
                $C$($\mu$g/mL) & 2.68 & 4.98 & 8.59 & 12.89 & 14.89 & 13.32 \\ \bottomrule \bottomrule
                $t$(h) & 18.0 & 24.0 & 36.0 & 48.0 & 72.0 \\ \hline
                $C$($\mu$g/mL) & 9.70 & 6.63 & 2.94 & 1.28 & 0.24 \\ \toprule 
            \end{tabular}
        \end{table}
    \end{question}
\end{frame}
...
\begin{frame}
    ...
    \]
    若$k_\mathrm{a}\gg k$，截距$\ln \frac{k_\mathrm{e}k_\mathrm{a}FX_0}{k_\mathrm{a}-k} \approx \ln k_\mathrm{e}FX_0$，
    所以
    \[
        X_\mathrm{u}^\infty = \frac{\mathrm{e}^{\text{截距}}}{k}
    \]
\end{frame}
...
\begin{frame}
    ...
    \[
        (X_\mathrm{u})_{t \to \infty} = \frac{(\mathrm{d}X_\mathrm{u}/\mathrm{d}t)_t}{k} \approx \frac{(\Delta X_\mathrm{u}/\Delta t)_t}{k} 
    \]
\end{frame}
    ...
    \begin{example}
        ...
        \begin{table}
            ...
            \begin{tabular}{cccccccccccc}
                ...
                $\Delta X_\mathrm{u}$(mg) & 4.12 & 5.26 & 5.98 & 17.86 & 14.30 & 8.32 & 4.88 \\ \toprule 
            \end{tabular}
        \end{table}
    \end{example}
\end{frame}
...
\begin{frame}[t]
    \frametitle{}
    \begin{redblock}{分组任务7：尿药排泄速率与时间的关系}
        某抗生素为单室模型药物，单次口服$X_0$mg后，于各时间段收集尿样，测定各时间段的尿药量，实验数据如下表所示。求消除速率常数$k$，消除半衰期$t_{1/2}$以及尿药排泄百分数。
    \end{redblock}
\end{frame}
...
\begin{frame}[t]
    ...
    \begin{figure}[htpb]
        \centering
        \includegraphics[width=\linewidth]{回归图}
    \end{figure}
\end{frame}
...
\begin{frame}[t]
    \frametitle{}
    \begin{redblock}{分组任务8：残数法求$k, k_\mathrm{a}$}
        某抗生素为单室模型药物，单次口服$X_0$mg后，于各时间段收集尿样，测定各时间段的尿药量，实验数据如下表所示。求消除速率常数$k$，吸收速率常数$k_\mathrm{a}$。
    \end{redblock}
\end{frame}
...
\begin{frame}[t]
    \frametitle{}
    \begin{redblock}{分组任务9：Wagner-Nelson法求$k_\mathrm{a}$}
        某抗生素为单室模型药物，单次口服$X_0$mg后，于各时间段收集尿样，测得尿药累积量如附件所示。用Wagner-Nelson法求吸收速率常数$k_\mathrm{a}$。
    \end{redblock}
\end{frame}
...
\begin{frame}
    \frametitle{消除速率常数k的计算}
    \begin{enumerate}
      \item 血药浓度法
      
      将最后5点血药浓度的对数与时间作线性回归，得
      \[
          k = 0.0875 \ \mathrm{h^{-1}}
      \]
      \item 尿药排泄速率法
      
      将末4点的$\frac{\Delta X_\mathrm{u}}{\Delta t}$取对数后对中点时间作线性回归，得
      \[
        k = 0.0659 \ \mathrm{h^{-1}}
      \]
      \item 尿药排泄亏量法
...
\begin{frame}
    \frametitle{由尿药排泄数据估算血药浓度}
    平均尿药排泄速率$\frac{\Delta X_{u}}{\Delta t}$与中点时间的血药浓度呈良好的线性关系，其斜率即为肾清除率。因此，本样例中得到的血药浓度与尿药排泄速率线性回归关系式如下：
    \[
        \frac{\Delta X_{u}}{\Delta t} = 2.8396C - 0.3885
    \]
    \begin{figure}[htpb]
        \centering
        \includegraphics[width=.4\linewidth]{血尿关系}
    \end{figure}
\end{frame}
