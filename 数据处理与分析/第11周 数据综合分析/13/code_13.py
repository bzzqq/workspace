...
def Task_One():
    fn=input('请输入文件名13.nino4.long.anom.data.csv：')
    try:

        df = pd.read_csv(fn)
        with open('nino4_dropnan.txt','w') as fp:
            fp.write('Date,Nino4\n')
            ...
            for year in range(len(counts)):#                
                ...
                for month in range(1,13):
                    ...
                    Nino4 = str(df.iloc[year,month])
                    s = y+'-'+m+'-01'+','+Nino4+'\n'
                    fp.write(s)
        df1 = pd.read_csv('nino4_dropnan.txt')
        df2 = df1.dropna()
        df2.to_csv('nino4_dropnan.txt',index=False)
        print('任务一完成')            
    except:
        print('读取文件出错')
def Task_Two():
     fn=input('请输入文件名nino4_dropnan.txt：')
     try:
        df=pd.read_csv(fn)        
        df_describe = df.describe()
        maxValue = df_describe.at['max','Nino4']
        minValue = df_describe.at['min','Nino4']
        meanValue = df_describe.at['mean','Nino4']
...
def Task_Three():
     ...
     try:
        ...
        x = pd.cut(df['Nino4'],category,right=False,labels=labels).rename('Label')
        df4 = pd.concat([df,x],axis=1)
        df4.to_csv('nino4_dropnan_result.csv',index=False)

        plt.figure()
        plt.pie([i[1] for i in list(x.value_counts().items())],
        labels=labels)

        plt.savefig('nino4_ pie.png',dpi=300)
        plt.show()
        ...
        print('读取文件出错')
def Task_Four():
     fn=input('请输入文件名nino4_dropnan_result.csv：')
     try:
         df3 = pd.read_csv(fn)
         import itertools
