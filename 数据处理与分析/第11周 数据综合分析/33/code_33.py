...
def dataPreprocessing():
        #读取数据
        fn = input('请输入所要打开的文件名称33.house.sale.price.csv:')
        try:
            df = pd.read_csv(fn)
            #查看前五行
            print(df.head(5))
...
def dataSelection():
        ...
        try:
            df = pd.read_csv(fn)
            df = df.dropna() #丢弃缺失值
            df_Selection = df.loc[:,['Id','Heating','LotArea','OverallCond','YrSold','SalePrice']]
            try:
...
def dataVisualization_1():
        ...
        try:
            df = pd.read_excel(fn)
            df_LotShape = df.loc[:,['Heating','unitPrice']]
            df_LotShape = df_LotShape.groupby(by='Heating',as_index=False).mean()
            df_LotShape = df_LotShape.sort_values(by='unitPrice',axis=0,ascending=False) #对每平方米售价进行降序排列
            df_LotShape.plot(x='Heating',kind='bar',figsize=(12,8),grid=0.1) #作柱状图
            plt.title('2006-2010年不同供暖系统房屋每平方米售价统计图') #命名标题
            plt.xlabel('供暖系统类型') #设置横轴标签
...
def dataVisualization_2():
        ...
        try:
            df = pd.read_excel(fn)
            df_LotShape = df.loc[:,['Heating','OverallCond']]
            df_LotShape = df_LotShape.groupby(by='Heating',as_index=False).mean()
            df_LotShape = df_LotShape.sort_values(by='OverallCond',axis=0) #对房屋整体状况进行升序排列
            df_LotShape.plot(x='Heating',kind='bar',figsize=(12,8),grid=0.1) #作柱状图
            plt.title('2006-2010年不同供暖系统的房屋整体状况统计图') #命名标题
            plt.xlabel('供暖系统类型') #设置横轴标签
...
def menu():
    print('                             【任务选择】\n'
          ...
          '|2.数据的预处理并挑取及导出                                                   |\n'
          '|3.归纳每平方米售价                                                   |\n'
          '|4.按供暖系统对每平方米售价分类及可视化                                |\n'
          '|5.按供暖系统对房屋整体状况分类及可视化                                |\n'
          '—————————————————————————————————————————————————————————————————————')
