...
# 分配学生到答辩组
# assigned_students = {"1组": [], "2组": [], "3组": [], "4组": [], "5组": [], "6组": [], "7组": []}
assigned_students = {f"{i}组": [] for i in range(1, 8)}
...
for attempt in range(max_attempts):
    ...
    for group in assigned_students:
        if len(assigned_students[group]) > 42:
            # 移动一个学生到另一个组
...
# 输出结果到Excel文件
results_df.to_excel('答辩分组结果.xlsx', index=False)
