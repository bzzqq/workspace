...
\begin{table}[ht]
    ...
    \cred{你是否遇到过以下困扰？}
    \begin{itemize}
      \item 输入身份证变成科学计数法，还原后最后几位变0
      \item 输入0开头的数字，0没有了
    \end{itemize}

    \cred{27个人类基因被重新命名，只因Excel总把它们自动纠正成日期}
    \begin{itemize}
      \item MARCH1：Membrane Associated Ring-CH Type Finger 1
      \item SEPT1
    \end{itemize}

    比起等待Excel的更新，要改变这恼人的现状，科学家们觉得还是重命名基因简单一些。

    一旦被自动转换，再去改成文本格式是无法恢复到原来的文本的，这就会导致大量数据的丢失。并且，目前没有任何办法关掉Excel默认设置下的自动格式转换。
